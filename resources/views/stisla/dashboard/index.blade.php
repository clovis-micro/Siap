@php
  ...
  $undangan = \App\Models\Modul::where('nama', 'undangan')->first();
  $undangan->ikon = str_replace('fa ', 'fas ', $undangan->ikon);

  $surat_masuk_count = \App\Arsip::where('jenis_dokumen_id', 1)->count();
  if (Auth::user()->role == 'admin') {
      $surat_masuk_count = \App\Arsip::where('jenis_dokumen_id', 1)
          ->where('kantor_id', Auth::user()->kantor->id)
          ->count();
  }
  $surat_keluar_count = \App\Arsip::where('jenis_dokumen_id', 2)->count();
  if (Auth::user()->role == 'admin') {
      $surat_keluar_count = \App\Arsip::where('jenis_dokumen_id', 2)
          ->where('kantor_id', Auth::user()->kantor->id)
          ->count();
  }
  $pegawai_count = \App\Arsip::where('jenis_dokumen_id', 3)->count();
  if (Auth::user()->role == 'admin') {
      $pegawai_count = \App\Arsip::where('jenis_dokumen_id', 3)
          ->where('kantor_id', Auth::user()->kantor->id)
          ->count();
  }
  $organisasi_count = \App\Arsip::where('jenis_dokumen_id', 4)->count();
  if (Auth::user()->role == 'admin') {
      $organisasi_count = \App\Arsip::where('jenis_dokumen_id', 4)
          ->where('kantor_id', Auth::user()->kantor->id)
          ->count();
  }
  $undangan_count = \App\Arsip::where('jenis_dokumen_id', 5)->count();
  if (Auth::user()->role == 'admin') {
      ...
      <div class="hero bg-primary text-white mb-3">
        <div class="hero-inner">
          ...
          <div class="mt-4">
            ...
            <a href="{{ route('arsip', ['undangan']) }}" class="btn btn-outline-white btn-lg btn-icon icon-left"><i class="{{ $undangan->ikon }}"></i> {{ $undangan->label }}</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-3 col-md-3 col-sm-6 col-12">
      <div class="card card-statistic-1">
        <div class="card-icon bg-info">
          <i class="{{ $undangan->ikon }}"></i>
        </div>
        <div class="card-wrap">
          <div class="card-header">
            <h4>{{ $undangan->label }}</h4>
          </div>
          <div class="card-body">
            {{ $undangan_count }}
          </div>
